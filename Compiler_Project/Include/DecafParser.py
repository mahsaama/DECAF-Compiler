...
Grammar = """
    ...
    expr : l_value "=" expr | constant | l_value | "this" | call | "(" expr ")" 
    ...
    double_constant : /(\d+\.(\d*)?((e|E)(\+|-)?\d+)?)/ | /(\d+(e|E)(\+|-)?\d+)/
    int_constant : /(0[x|X][0-9a-fA-F]+)/ | /(\d+)/
...
def parserSyntaxError(input_code):
    try:
